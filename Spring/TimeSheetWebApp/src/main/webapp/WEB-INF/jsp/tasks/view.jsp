...
<%--@elvariable id="unassigned" type="java.util.List<com.ir.timesheet.model.Employee>"--%>

<html>
<head>
<title>Task page</title>
<link rel="stylesheet" href="/TimeSheetSample/resources/styles.css"
	type="text/css">
</head>
<body>
	<h2>Task info</h2>
	<div id="list">
		<ul>
			...
			<li><label for="manager">Manager:</label> <input name="manager"
				id="manager" value="${task.manager.name}" disabled="true" /></li>
					...
					<c:forEach items="${task.assignedEmployees}" var="emp">
						<tr>
							<sf:form action="${task.id}/employees/${emp.id}" method="delete">
								<td><a href="../employees/${emp.id}" id="href-${emp.id}">${emp.name}</a>
								</td>
								<td><input type="submit" value="Remove"
									id="remove-${emp.id}" /> <script
										src="/TimeSheetSample/resources/jquery.js"></script> <script
				...
				<table id="unassigned" class="task-table">
					<tr>
						<sf:form method="put" id="add-form">
							...
							<td><input type="submit" value="Add" id="add-employee" /> <script
									src="/TimeSheetSample/resources/jquery.js"></script> <script
									type="text/javascript">
