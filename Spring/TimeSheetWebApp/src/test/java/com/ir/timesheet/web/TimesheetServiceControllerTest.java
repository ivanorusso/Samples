...
/**
 * This test relies on fact that DAOs and Services are tested individually. Only
 * compares, if controller returns the same as individual services.
 */
...
public class TimesheetServiceControllerTest extends DomainAwareBase {
	...
	public void testShowManagerTasks() {
		...
		assertEquals(timesheetService.tasksForManager(manager), model.asMap()
				.get("tasks"));
	}
	...
	public void testShowEmployeeTasks() {
		...
		assertEquals(timesheetService.tasksForEmployee(employee), model.asMap()
				.get("tasks"));
	}
}